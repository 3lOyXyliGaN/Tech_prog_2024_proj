#include "TCPsrvr.h" // включаем h-файл
#include "FuncForSrvr.h" // включаем функции для сервера
...
void TcpServer::slotServerRead()
{
    ...
    if (!res.isEmpty())
    {
        socket->write(FuncForSrvr::parse(res)); // Отправка проанализированного результата
    }
}
